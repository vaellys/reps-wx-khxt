...
<body>
	<reps:container>
		<reps:panel id="myform" dock="top" action="" formId="xform"
			validForm="true">
			<reps:formcontent>
				<input type="hidden" value="${levelWeight.id }" name="id">
				<reps:formfield label="权重名称" labelStyle="width:20%;"
					textStyle="width:20%;">
					<reps:input name="name" minLength="2" maxLength="20" readonly="true"
							...
							<c:forEach var="level" items="${levelList}" varStatus="status">
								<tr>
									...
									<td><input class="txtInput integernum required"
										...
										value="" levelId="${level.id }" readonly="readonly"></td>
								</tr>
							</c:forEach>
						</tbody>
					</table>
