	...
	<reps:panel title="" id="top" dock="top" method="post" action="list.mvc" formId="queryForm">
		...
		<reps:formcontent parentLayout="true" style="width:80%;" columns="2">
			...
			<reps:formfield label="考核对象" labelStyle="width:23%;" textStyle="width:30%;">
				<reps:select dataSource="${levelMap}" name="bkhrId" >${sheet.bkhr.id }</reps:select>
			</reps:formfield>
			<reps:formfield label="考核年月" labelStyle="width:20%;" textStyle="width:27%;" >
			 ...
			 <reps:formfield label="考核进度" labelStyle="width:23%;" textStyle="width:30%;">
				<reps:select dataSource="${status}" name="progress" >${sheet.progress }</reps:select>
			</reps:formfield> 
		</reps:formcontent>
		<reps:querybuttons>
	...
	<reps:panel id="mybody" dock="center">
		<reps:grid id="itemList" items="${list}" form="queryForm" var="sheet" pagination="${pager}" flagSeq="true">
			<reps:gridrow>
				...
				<reps:gridfield title="考核人权重" width="25" align="center">${sheet.weight}</reps:gridfield>
				
				<reps:gridfield title="填报截止时间" width="15" align="center">${sheet.endEate}</reps:gridfield>
				
				<reps:gridfield title="考核对象人数" width="5" align="center">${sheet.bkhrCount}</reps:gridfield>
